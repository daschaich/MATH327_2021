...
# comparing direct evaluation to the central limit theorem
...
print("For each spin the mean gain is %.3g " % mean, end=''),
print("with standard deviation %.5g" % np.sqrt(var))
...
for W in range(N + 1):
  ...
  # For optional checking
  if gain[W] < 0.0:
    print("Probability %.4g to lose %d" % (prob[W], -1.0 * gain[W]))
  else:
    print("Probability %.4g to gain %d" % (prob[W], gain[W]))
print("Probabilities sum to %.4g" % np.sum(prob))
...
for W in range(N + 1):
  ...
  # For optional checking
  if gain[W] < 0.0:
    print("Probability %.4g to lose %d" % (const[W], -1.0 * gain[W]))
  else:
    print("Probability %.4g to gain %d" % (const[W], gain[W]))
print("Probabilities sum to %.4g" % np.sum(const))
...
for W in range(N + 1):
  ...
  # For optional checking
  if gain[W] < 0.0:
    print("Probability %.4g to lose %d" % (integ[W], -1.0 * gain[W]))
  else:
    print("Probability %.4g to gain %d" % (integ[W], gain[W]))
