...
\begin{align}
              \mu = \vev{X} = & \sum_{X \in A} X P(X) \\
  \si^2 = \vev{(X - \mu)^2} = & \sum_{X \in A} (X - \mu)^2 P(X).
\end{align}
...
Keeping the measurement fixed, we can call the resulting outcome space $B$.
\newpage % WARNING: FORMATTING BY HAND
\noindent For $R = 4$, what are some representative outcomes in the set $B$?
What is the total size of $B$?
\begin{mdframed}
  \ \\[100 pt]
\end{mdframed}

Each outcome in $B$ contains $R$ different $X^{(r)} \in A$, one for each repetition $r = 1, \cdots, R$.
\TODO{...}
% ------------------------------------------------------------------
