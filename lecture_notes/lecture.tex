...
\newcommand{\moddate}{Last modified 11 Jan.~2021}
...
\cfoot{{\small MATH327}\hfill \thepage\hfill {\small \moddate}}
% ------------------------------------------------------------------
...
Using this result and the relation $\left(\sum_i a_i\right)\left(\sum_j b_j\right) = \sum_{i, j} \left(a_i b_j\right)$, express the following quantity in terms of \si and $R$:
\begin{mdframed}
...
Starting from this density, a probability is determined by integrating over a given interval, which we can call $[a, b]$.
This integral produces the probability that the outcome $X$ lies within this interval,
\begin{equation*}
  P\left(a \leq X \leq b\right) = \int_a^b p(x) dx.
\end{equation*}

We similarly generalize the definition of an expectation value (\eq{eq:expect_disc}) to integrate over the probability distribution,
\begin{equation}
  \label{eq:expect_cont}
  \vev{f(X)} = \int f(x) p(x) dx
\end{equation}
By omitting the limits on the integral we implicitly integrate over the entire domain, in this case $\int dx = \int_{-\infty}^{\infty} dx$.
\TODO{mean, stdev,...}
\TODO{...}
% ------------------------------------------------------------------
