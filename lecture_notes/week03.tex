...
As we might have expected based on last week's look at the micro-canonical ensemble, all micro-states with the same energy have the same probability in thermodynamic equilibrium.
% ------------------------------------------------------------------
...
\begin{shaded}
  ...
  \begin{equation}
    \label{eq:heat_cap}
    c_v = \pderiv{}{T} \vev{E} \geq 0.
  \end{equation}
\end{shaded}

In a homework assignment you will show that $c_v$\footnote{The subscript indicates that the volume of the system is kept fixed; details will wait until we properly introduce the volume over the next couple of weeks.} is always non-negative (and generically positive), by deriving a so-called \textbf{fluctuation--dissipation} (or \textbf{fluctuation--response}) \textbf{relation}.
This will be a special case of a \href{https://en.wikipedia.org/wiki/Fluctuation-dissipation_theorem}{more general theorem}, and will relate the fluctuations of the internal energy around its expectation value, $\sum_i \left(E_i - \vev{E}\right)^2$, to the energy's \textit{response} to a change in temperature, $\pderiv{}{T} \vev{E}$.
The (generically) positive heat capacity confirms our expectation from everyday experiences that higher temperatures produce larger (average) internal energies.
...
Last week we saw that the micro-canonical entropy depended logarithmically on the number of micro-states.
Now \eq{eq:canon_entropy} contains a similar logarithm of the partition function, which we identified as a fundamental quantity in the canonical ensemble.
This motivates the definition We can 
...
\begin{mdframed}
  $\displaystyle \pderiv{}{T} \log Z = $ \\[100 pt]
  \ \\[100 pt]
\end{mdframed}
Combining this result with \eq{eq:canon_entropy} and the expression for $\vev{E}$ from an earlier gap, \TODO{...}
