...
We can gain a more intuitive interpretation of these findings by inspecting them in terms of the total time $T$ taken by the random walk (\eq{eq:RWtime}).
Inserting $N = T / \De t$ into \eq{eq:RWmean},
\begin{equation*}
  \vev{x} = \frac{T}{\De t} (2p - 1) = \frac{2p - 1}{\De t} T \equiv \vdr T,
\end{equation*}
we define the \textbf{drift velocity}
\begin{equation}
  \vdr = \frac{2p - 1}{\De t} = \frac{N(2p - 1)}{T} = \frac{\vev{x}}{T}.
\end{equation}
The standard deviation provides a measure of the \textit{absolute} uncertainty in the final position of the walker,
\begin{equation*}
  \De x = \si = \sqrt{\vev{x^2} - \vev{x}^2} = 2\sqrt{Npq} = 2\sqrt{\frac{pq}{\De t}} \sqrt{T},
\end{equation*}
which also increases as time passes, for non-trivial (non-zero) $p$ and $q$.
\TODO{Diffusion...}
