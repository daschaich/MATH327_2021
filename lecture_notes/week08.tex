...
\begin{equation*}
  I = \int_0^{\infty} \frac{x^3}{e^x - 1} dx = \Gamma(4) \zeta(4) = \frac{\pi^4}{15}.
\end{equation*}
...
It would be ideal (sorry) if we could put this in a form as simple as \eq{eq:ideal_energy} for the energy of an $N$-particle ideal gas in the canonical ensemble.
Now that we are working in the grand-canonical ensemble, we would need to compute the average particle number $\vev{N}_{\text{ph}}$, building on the general Bose--Einstein result
\begin{equation*}
  \vev{N}_{\text{BE}} = \sum_{\ell = 0}^L \vev{n_{\ell}} = \sum_{\ell = 0}^L \frac{1}{e^{\be (E_{\ell} - \mu)} - 1}
\end{equation*}
that we derived last week.
Again setting $\mu = 0$ and recalling the two-fold degeneracy of each photon energy level,
